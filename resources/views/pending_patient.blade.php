...
@foreach($pendingAppointments as $appt)
    @php
        $pat = $patients[$appt->user_email];
    @endphp
    @if($pat)
    <div class="doctor-card">
        <img src="{{ asset('images/pic.png') }}" alt="{{ $pat->name }}" class="doctor-image">
        <div class="doctor-name">{{ $pat->name }}</div>
        <div class="doctor-specialty">Email: {{ $pat->email }}</div>
        <div class="doctor-specialty">Gender: {{ $pat->sex }}</div>
        <div class="doctor-specialty">Age: {{ $pat->age }}</div>
        <div class="doctor-specialty">Scheduled: {{ $appt->time }}</div>

        <form action="{{ route('consultation.confirm', $appt->id) }}" method="POST" style="display:inline-block;">
            @csrf
            <button type="submit" class="book-button">Confirm</button>
        </form>

        <form action="{{ route('consultation.cancel', $appt->id) }}" method="POST" style="display:inline-block;">
            @csrf
